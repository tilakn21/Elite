...
class _AdminCalendarScreenState extends State<AdminCalendarScreen> {
  ...
  Map<DateTime, List<AdminJob>> _jobsMap = {};
  ...
  Map<DateTime, List<AdminJob>> _groupJobsByDate(List<AdminJob> jobs) {
    Map<DateTime, List<AdminJob>> data = {};
    for (var job in jobs) {
      try {
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),            Row(
              children: [
                _buildJobDetailChip('Receptionist', job.receptionist?.isNotEmpty ?? false),
                const SizedBox(width: 8),
                _buildJobDetailChip('Salesperson', job.salesperson?.isNotEmpty ?? false),
                const SizedBox(width: 8),
                _buildJobDetailChip('Design', job.design?.isNotEmpty ?? false),
              ],
            ),
            const SizedBox(height: 8),
            Row(
              children: [
                _buildJobDetailChip('Production', job.production?.isNotEmpty ?? false),
                const SizedBox(width: 8),
                _buildJobDetailChip('Printing', job.printing?.isNotEmpty ?? false),
                const SizedBox(width: 8),
                _buildJobDetailChip('Accountant', job.accountant?.isNotEmpty ?? false),
              ],
            ),
          ],
        ),
                      ...
                      child: SingleChildScrollView(
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 8),
                            Text(
                              'Click on any date to view scheduled jobs',
                              style: TextStyle(
                                fontSize: 16,
                                color: Colors.grey[600],
                              ),
