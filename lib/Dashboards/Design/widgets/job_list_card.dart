...
class JobListCard extends StatelessWidget {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if (jobs.isNotEmpty && !isMobile) _buildApprovedJobsSection(context),
            if (jobs.isNotEmpty && !isMobile) const SizedBox(height: 24),
            if (!isMobile) _buildJobListHeader(context),
            if (!isMobile) const Divider(height: 1, thickness: 1, color: Color(0xFFEEEEEE)),
  ...
  Widget _buildApprovedJobsSection(BuildContext context) {
    final approvedJobs = jobs.where((job) => job.status == JobStatus.approved).toList();
            ...
            Row(
              children: [
                CircleAvatar(
                  ...
                  child: Text(
                    job.clientName.isNotEmpty ? job.clientName.substring(0, 1).toUpperCase() : '?',
  ...
  Widget _buildJobListItem(BuildContext context, Job job, {bool isMobile = false}) {
